...
const NEWS_SITE_DOMAINS: string[] = [
    // US National News
...
function hideHeadlinesImmediately(): void {
...
function restoreHeadlineColors(): void {
...
let cachedReplacements: Replacement[] = [];
const replacementCache = new Map<string, string>(); // original text -> new text
let domObserver: MutationObserver | null = null;
let loadingBanner: HTMLDivElement | null = null;

// Show loading banner at top of page
function showLoadingBanner(message: string): void {
    if (!loadingBanner) {
...
function extractAboveFoldContent(): TextBlock[] {
    ...
    const allElements = document.querySelectorAll<HTMLElement>('h1, h2, h3, h4, h5, h6, p, span, a, li, div');
...
const FALLBACK_REPLACEMENTS: FallbackRule[] = [
    // People/Entities
...
function applyFallbackReplacements(): void {
...
function findAndReplaceText(searchText: string, replaceText: string): number {
...
function reapplyCachedReplacements(): void {
...
async function filterNegativity(): Promise<void> {
    ...
    const pageContent: PageContentBlock[] = textBlocks.map((block, index) => ({
        index,
        text: block.text,
        tag: block.tag,
    ...
    try {
        const request: FilterContentRequest = {
            action: 'filterContent',
            pageContent,
            url: window.location.href,
        };
        const response: FilterContentResponse = await chrome.runtime.sendMessage(request);
        ...
        if (response.success) {
            ...
            setTimeout(() => {
                const stillLoading = document.querySelectorAll<HTMLElement>('.positivity-loading');
...
chrome.runtime.onMessage.addListener(
    (request: ExtensionRequest, _sender, sendResponse: (response: { success: boolean }) => void) => {
